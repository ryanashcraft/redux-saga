...
import {
  ...
  i as uid,
  j as array$1,
  k as assignWithSymbols,
  l as makeIterator,
  d as createSetContextWarning,
  m as shouldCancel,
  n as shouldTerminate,
  o as createAllStyleChildCallbacks,
  p as shouldComplete,
  q as asyncIteratorSymbol,
  r as wrapSagaDispatch,
  s as log,
  e as identity,
} from './chunk-e16e13ae.js'
import {
  CHANNEL_END_TYPE,
  MATCH,
  MULTICAST,
...
function proc(env, iterator$$1, parentContext, parentEffectId, meta, cont) {
  ...
  **/
  ...
  function runEffect(effect, effectId, currCb) {
    ...
    **/
    if (promise(effect)) {
      ...
      if (type === TAKE) runTakeEffect(payload, currCb)
      else if (type === PUT) runPutEffect(payload, currCb)
      else if (type === ALL) runAllEffect(payload, effectId, currCb)
      else if (type === RACE) runRaceEffect(payload, effectId, currCb)
      else if (type === CALL) runCallEffect(payload, effectId, currCb)
      else if (type === CPS) runCPSEffect(payload, currCb)
      else if (type === FORK) runForkEffect(payload, effectId, currCb)
      else if (type === JOIN) runJoinEffect(payload, currCb)
      else if (type === CANCEL$1) runCancelEffect(payload, currCb)
      else if (type === SELECT) runSelectEffect(payload, currCb)
      else if (type === ACTION_CHANNEL) runChannelEffect(payload, currCb)
      else if (type === FLUSH) runFlushEffect(payload, currCb)
      else if (type === CANCELLED) runCancelledEffect(payload, currCb)
      else if (type === GET_CONTEXT) runGetContextEffect(payload, currCb)
      else if (type === SET_CONTEXT) runSetContextEffect(payload, currCb)
      else currCb(effect)
    } else {
  ...
  function newTask(id, meta, cont) {
    ...
    (_task2.id = id),
    (_task2.meta = meta),
    (_task2._deferredEnd = null),
    (_task2.toPromise = function toPromise() {
    ...
    (_task2.cont = cont),
    (_task2.joiners = []),
    (_task2.cancel = cancel),
    (_task2._isRunning = true),
    (_task2._isCancelled = false),
    (_task2._isAborted = false),
    (_task2._result = undefined),
    (_task2._error = undefined),
    (_task2.isRunning = function isRunning() {
